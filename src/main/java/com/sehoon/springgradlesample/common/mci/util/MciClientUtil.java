...
public class MciClientUtil {
	...
	public static <T> T send(MciChannelConst channel, Object inVo, Class<T> outClass, int timeoutMs) throws Exception {
		// 채널별로 연동 타입이 있음
		log.info(channel.getValue()+"-type");
		String chType = MciUtil.getMciProp(channel.getValue()+"-type");
	...
	private static <T> T sendJSON(MciChannelConst channel, Object inVo, Class<T> outClass, int timeoutMs) throws Exception {
		...
		String chUrl = MciUtil.getMciProp(channel.getValue() + "-base-url");
		...
		*/
		...
		tgrmCmnnhddvValu.setEnvrTypeCd(MciUtil.getMciProp("envr-type-cd"));
	...
	private static <T> T sendHFLD(MciChannelConst channel, Object inVo, Class<T> outClass, int timeoutMs) throws Exception {
		...
		// 대외 MCI 전용 파라메터 추가
		String frbuCd = MciUtil.getMciProp(channel.getValue()+"-frbu-cd");		// 대외기관코드
		String cmouDutjCd = MciUtil.getMciProp(channel.getValue()+"-cmou-dutj-cd");	// 대외업무코드
		String chEnc = MciUtil.getMciProp(channel.getValue()+"-encode");	// FLD 인코딩 - 디폴트는 EUC-KR
		...
		if (glbId == null || glbId.length() < 1) {
			String appliDutjCd = itrfId.substring(0, 3); // 어플리케이션업무코드(3)
			tgrmCmnnhddvValu.setGlbId(MciClientUtil._nextNlGlbId(appliDutjCd));		// Mci 대내 전문과 중복되지않도록 MciUtil 글로벌ID를 공유함 !!!!
	...
	static synchronized String _nextNlGlbId(String appliDutjCd) throws Exception {
		String hostname = StringUtils.defaultString(MciUtil.getMciProp("hostname"), "");
		...
		if (s_nlGlbId != null && s_nlGlbId.startsWith(next)) {
			seq = MciUtil.parseToInt(s_nlGlbId.substring(29)) + 1;
		}
