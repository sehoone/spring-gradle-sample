...
public class MciCommHeaderVo{
	...
	public byte[] marshalFld(String encode){
        ...
        try {
            ...
            try	{
                ...
                log.error("marshalFld out close Error", ie);
           }
            try	{
                if (bout != null) bout.close();
           } catch (IOException ie) {
                log.error("marshalFld bout close Error", ie);
           }
        }
    ...
    public void unMarshalFld( byte[] bytes , String encode) throws Exception{
        try{ 
            ...
            String errorLine = CcFwUtil.getTrimmedString( bytes, 0, bytes.length, encode );
            log.error("unMarshalFld Error:["+ errorLine+"]", e);
            throw e;
        } 
    }
}
